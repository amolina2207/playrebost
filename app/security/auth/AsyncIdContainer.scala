...
// The Id type is a String, defined on BaseAuthConfig where this class is instanciated
trait AsyncIdContainer[Id] {
  ...
  def checkAccessAuthorized(implicit request: RequestHeader, context: ExecutionContext, timeoutInSeconds: Int): Future[Option[Boolean]]
...
object AsyncIdContainer {
  def apply[A](underlying: IdContainer[A]): AsyncIdContainer[A] = new AsyncIdContainer[A] {
    ...
    def checkAccessAuthorized(implicit request: RequestHeader, context: ExecutionContext, timeoutInSeconds: Int): Future[Option[Boolean]] =
      future(underlying.checkAccessAuthorized(request, context, timeoutInSeconds))
      
//    def extractToken(implicit request: RequestHeader): Future[Option[String]] =
//      future(underlying.extractToken(request))
